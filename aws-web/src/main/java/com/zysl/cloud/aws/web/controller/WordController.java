...
public class WordController extends BaseController implements WordSrv {
	...
	public BaseResponse<WordToPDFDTO> changeWordToPdf(@RequestBody WordToPDFRequest request){
		return ServiceProvider.call(request, WordToPDFRequestV.class,WordToPDFDTO.class,req->{
			...
			byte[] outBuff = wordService.changeWordToPDF(fileName, s3ObjectBO.getBodys(),false, request.getTextMark());
			log.info("===changeToPDF===outBuff,fileName:{},outBuff.length:{}", fileName,outBuff == null ? 0 : outBuff.length);
...
//	public BaseResponse<WordToPDFDTO> changeWordToPdf2(@RequestBody WordToPDFRequest request){
